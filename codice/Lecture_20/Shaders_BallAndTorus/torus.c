...
void fillTorVertexArray(Vertex torVertices[(TOR_LONGS + 1) * (TOR_LATS + 1)]) {
    ...
    for (j = 0; j <= TOR_LATS; j++)
        ...
        {
            torVertices[k].coords.x = ( TOR_OUTRAD + TOR_INRAD *
                cos( (-1 + 2*(float)j/TOR_LATS) * M_PI ) ) * cos( (-1 + 2*(float)i/TOR_LONGS) * M_PI );
            torVertices[k].coords.y = ( TOR_OUTRAD + TOR_INRAD *
                cos( (-1 + 2*(float)j/TOR_LATS) * M_PI ) ) * sin( (-1 + 2*(float)i/TOR_LONGS) * M_PI );
