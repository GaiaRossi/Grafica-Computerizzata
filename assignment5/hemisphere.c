...
void fillHemVertexArray(Vertex hemVertices[(HEM_LONGS + 1) * (HEM_LATS + 1)], float radius) {
    int i, j, k;

    k = 0;
    for (j = 0; j <= HEM_LATS; j++)
        for (i = 0; i <= HEM_LONGS; i++) {
            hemVertices[k].coords.x = radius * sin( (float)j/HEM_LATS * M_PI/2.0 ) *
                sin( 2.0 * (float)i/HEM_LONGS * M_PI );
            hemVertices[k].coords.y = radius * cos( (float)j/HEM_LATS * M_PI/2.0 );
            hemVertices[k].coords.z = radius * sin( (float)j/HEM_LATS * M_PI/2.0 ) *
...
void fillHemisphere(Vertex hemVertices[(HEM_LONGS + 1) * (HEM_LATS + 1)],
                    ...
                    float radius) {
    fillHemVertexArray(hemVertices, radius);
